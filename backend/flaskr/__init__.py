...
def create_app(test_config=None):
  ...
  def retrieve_questions():
    ...
    current_questions, current_categories = paginate_questions(request, selection)
    categories = Category.query.order_by(Category.id).all()
    data = {}
    for cat in categories:
      data[cat.id] = cat.type
  ...
  def get_one_quiz_question():
    ...
    try:
      ...
      if categoryId == 0:
        ...
        question = Question.query.filter(Question.category == categoryId,  ~Question.id.in_(previous_questions)).first()
